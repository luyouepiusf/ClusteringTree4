...
predict_distance<-function(survival_tree,matrix_numeric,matrix_factor){
  
  a_predict_node<-predict_node(survival_tree,matrix_numeric,matrix_factor)
  ...
  ind_distance<-matrix(NA,nrow(matrix_numeric),nrow(matrix_numeric))
  for(ii in 1:nrow(matrix_numeric)){
    for(jj in 1:nrow(matrix_numeric)){
      ind_distance[ii,jj]<-node_distance[a_predict_node$id[ii],a_predict_node$id[jj]]
    }
  }
  return(
    list(node_distance=node_distance,
         ind_distance=ind_distance,
         ind_predict=a_predict_node))
}
